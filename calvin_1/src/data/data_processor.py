...
from data.lunarcrush_api import LunarCrushAPI
from src.config.config import config
from utils.logger import log_manager
from src.database.production_db import ProductionDBManager, OHLCVData
...
class DataProcessor:
    ...
    def prepare_ml_data(
        ...
        # Target is now the percentage change
        ...
        # IMPORTANT: Fit the price_scaler on the percentage change targets
        self.price_scaler.fit(targets)
        
        # Scale the data
        scaled_features = self.feature_scaler.transform(features)
        scaled_targets = self.price_scaler.transform(targets).flatten()
        ...
        X, y, prices_at_sequence_end = self._create_sequences_with_price(
            scaled_features, scaled_targets, prices_for_inverse, sequence_length
        )
        ...
        logger.info(f"Final data shapes - X_train: {X_train.shape}, X_test: {X_test.shape}, y_train: {y_train.shape}, y_test: {y_test.shape}")
    ...
    def calculate_social_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in result.columns:
            if col not in df.columns and pd.api.types.is_numeric_dtype(result[col]):
                ...
                result[col] = result[col].interpolate(method='linear', limit_direction='both').ffill().bfill()
    ...
    def add_market_regime_features(self, df: pd.DataFrame, 
        ...
        for col in regime_cols:
            if result[col].dtype == 'object':
                ...
                # For numeric columns, forward fill then backfill

...
const DiscordAuthButton = () => {
  const { discordUser, userTier, isLoading, isAuthenticated, canAccessVault, getAccessLevel } = useDiscordAuth()
  ...
  if (!isAuthenticated) {
    return (
      <button
        ...
        className="flex items-center gap-3 px-6 py-3 bg-[#5865F2] hover:bg-[#4752C4] text-white rounded-lg font-medium transition-colors duration-200 shadow-lg hover:shadow-xl"
      >
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-2">
            <span className={`text-xs px-2 py-1 rounded-full font-medium ${
              ...
              {userTier === 'CALVIN_WHALE' ? '$CALVIN Whale' : 'No Access'}
            </span>
          </div>
        </div>
      </div>

...
pub fn current_nav_usdc<'info>(
    ...
    for (i, token_account) in token_accounts.iter().enumerate() {
        ...
        let price = price_feed.get_price().ok_or(error!(ErrorCode::InvalidPriceType))?;
        ...
        let price_in_usdc = price.price
            .checked_mul(10i64.pow(price.expo.unsigned_abs()))
...
pub fn check_price_staleness(
    ...
    if price_feed.price_type != pyth_sdk_solana::state::PriceType::Price {
        return Err(error!(ErrorCode::InvalidPriceType));
    }
    
    // Check price status
    if price_feed.price_status != PriceStatus::Trading {
